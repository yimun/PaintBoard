...
namespace Utils
{
	...
	//
	//
	BOOL CMD5Checksum::IsValidAddress(const void* lp, UINT nBytes, BOOL bReadWrite/*= TRUE*/)
	{
	...
	exception.
	*****************************************************************************************/
	BOOL CMD5Checksum::GetMD5(LPCTSTR lpFilePath, LPTSTR lpRet, LPDWORD pdwTickCount/* = NULL*/)
	{
		DWORD dwTempTick = GetTickCount();
		//Handle for the file detecting
		HANDLE hFile;
		//Handle for the mapping file detecting
		HANDLE hMapping;
		LPVOID pBasePointer;
...
#ifdef UNDER_CE
		hFile = CreateFileForMapping(lpFilePath, dwCreateFileAccess, dwCreateFileShare, 0,
			OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
#else
		hFile = CreateFile(lpFilePath, dwCreateFileAccess, dwCreateFileShare, 0,
			OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);		
#endif
		...
		if (!(hMapping = CreateFileMapping(hFile, 0, dwMapProtect, 0, 0, 0)))
		{
			CloseHandle(hFile);
			CloseHandle(hMapping);
			return FALSE;
		}
		if (!(pBasePointer = ::MapViewOfFile(hMapping, dwViewAccess, 0, 0, 0)))
		{
			CloseHandle(hFile);
			CloseHandle(hMapping);
			return FALSE;
		...
		DWORD dwLowSize = 0;
		dwLowSize = (ULONG)GetFileSize(hFile, NULL);
		GetMD5((LPBYTE)pBasePointer, dwLowSize, lpRet);
		...
		//
		dwTempTick = GetTickCount() - dwTempTick;
		if(pdwTickCount)
		{
			*pdwTickCount = dwTempTick;
		}
		//
		return TRUE;
	}
	...
	*****************************************************************************************/
	BOOL CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength, LPTSTR lpRet, LPDWORD pdwTickCount /*= NULL*/)
	{
		DWORD dwTempTick = GetTickCount();
		//entry invariants
	...
	*****************************************************************************************/
	void CMD5Checksum::ByteToDWord(DWORD* Output, BYTE* Input, UINT nLength)
	{
		//entry invariants
		assert( nLength % 4 == 0 );
		assert( IsValidAddress(Output, nLength/4, TRUE) );
		assert( IsValidAddress(Input, nLength, FALSE) );
	...
	*****************************************************************************************/
	void CMD5Checksum::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
	{
		//entry invariants
		assert( nLength % 4 == 0 );
		assert( IsValidAddress(Output, nLength, TRUE) );
		assert( IsValidAddress(Input, nLength/4, FALSE) );
	...
	*****************************************************************************************/
	void CMD5Checksum::Final(LPTSTR lpRet)
	{
		//Save number of bits
		BYTE Bits[8];
		...
		for ( int i=0; i < nMD5Size; i++) 
		{
			...
			{
				wsprintf(Str, _T("0%X"),lpszMD5[i]);
			}
			else 
			{
				wsprintf(Str, _T("%X"),lpszMD5[i]);
			}
